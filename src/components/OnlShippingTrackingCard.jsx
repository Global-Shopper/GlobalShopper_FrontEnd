...
import OnlineTrackingDialog from './OnlineTrackingDialog'

export default function OnlShippingTrackingCard({ data }) {
  if (!data) return null

  const trackingNumber = data.tracking_number || data.order_number || null
  const events = Array.isArray(data?.origin_info?.trackinfo) ? data.origin_info.trackinfo : []
  console.log(data)
  const statusRaw = (data.delivery_status || 'UNKNOWN').toString()

  // Helpers for formatting time/date like the offline card
  ...
  // Normalize status to milestone steps
  const normalizeStatus = (s) => {
    const u = String(s || '').toUpperCase()
    if (u.includes('OUT') && u.includes('DELIVERY')) return 'OUT_FOR_DELIVERY'
    if (u.includes('DELIVERED')) return 'DELIVERED'
    if (u.includes('TRANSIT')) return 'IN_TRANSIT'
    if (u.includes('PICKUP') || u.includes('PICKED')) return 'PICKED_UP'
    if (u.includes('EXCEPTION') || u.includes('UNDELIVERED') || u.includes('FAIL')) return 'EXCEPTION'
    return u || 'UNKNOWN'
  }
    ...
    <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="px-4 py-3">
        <div className="flex items-center justify-between text-sm text-slate-700">
          ...
          <OnlineTrackingDialog data={data} />
        </div>
      </div>
            ...
            <div className="nss-comp-tracking-content">
              <ul className="space-y-3">
                {events.slice(0, 10).map((ev, idx) => {
                  const desc = (ev.tracking_detail || ev.checkpoint_delivery_status || '').toString().trim()
                  const isCurrent = idx === 0
                  return (
                    <li key={`${ev.checkpoint_date}-${idx}`} className="nss-comp-tracking-item">
                      <div className="flex items-start gap-3">
                        {/* Left time */}
                        <div className="left w-24 text-right">
                          <div className="time">
                            <div className="second text-xs text-slate-600 font-medium">{fmtTimeOnly(ev.checkpoint_date)}</div>
                            <div className="day text-[11px] text-slate-500">{fmtDateShort(ev.checkpoint_date)}</div>
                          </div>
                        </div>
                        {/* Mid icon + line */}
