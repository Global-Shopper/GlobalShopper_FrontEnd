    ...
    requestItems: Yup.array()
      .of(
        Yup.object({
          ...
          productURL: Yup.string().trim().nullable().transform((value) => (value === '' ? null : value)).url('Đường dẫn không hợp lệ'),
...
const PurchaseRequestEditOffline = ({ id, data }) => {
  ...
  const initialValues = useMemo(() => {
    const pr = data || {}
    const firstSR = Array.isArray(pr.subRequests) && pr.subRequests.length ? pr.subRequests[0] : {}
    const items = (pr.subRequests || []).flatMap((sr) => sr?.requestItems || []).concat(pr?.requestItems || [])
    const parsedContact = parseContactFields(firstSR.contactInfo || pr.contactInfo || [])
    return {
      shippingAddressId:
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        <Card className="shadow-lg p-2 py-4">
          <CardHeader className="bg-gradient-to-r from-orange-600 to-orange-700 text-white rounded-lg">
            <CardTitle className="text-xl">Chỉnh sửa yêu cầu mua hàng (Offline)</CardTitle>
          </CardHeader>
              ...
              {({ values, handleChange, handleBlur, setFieldValue, setFieldTouched, errors, touched, submitCount }) => (
                <Form className="space-y-6">
                  <div className="space-y-2">
                    ...
                    {(touched.shippingAddressId || submitCount > 0) && errors.shippingAddressId && (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <Label htmlFor="shopName">Tên cửa hàng <span className="text-red-600">*</span></Label>
                        <Input id="shopName" name="shopName" value={values.shopName} onChange={handleChange} onBlur={handleBlur} placeholder="Tên cửa hàng" />
                        {(touched.shopName || submitCount > 0) && errors.shopName && <div className="text-sm text-red-500">{errors.shopName}</div>}
                      </div>
                      <div className="space-y-1">
                        <Label htmlFor="shopEmail">Email cửa hàng</Label>
                        <Input id="shopEmail" name="shopEmail" type="email" value={values.shopEmail} onChange={handleChange} onBlur={handleBlur} placeholder="Email cửa hàng (không bắt buộc)" />
                        {(touched.shopEmail || submitCount > 0) && errors.shopEmail && <div className="text-sm text-red-500">{errors.shopEmail}</div>}
                      </div>
                      <div className="space-y-1">
                        <Label htmlFor="shopAddress">Địa chỉ cửa hàng <span className="text-red-600">*</span></Label>
                        <Input id="shopAddress" name="shopAddress" value={values.shopAddress} onChange={handleChange} onBlur={handleBlur} placeholder="Địa chỉ cửa hàng" />
                        {(touched.shopAddress || submitCount > 0) && errors.shopAddress && <div className="text-sm text-red-500">{errors.shopAddress}</div>}
                      </div>
                      <div className="space-y-1 md:col-span-2">
                        <Label htmlFor="shopWebsite">Link website hoặc mạng xã hội của cửa hàng</Label>
                        <Input id="shopWebsite" name="shopWebsite" value={values.shopWebsite} onChange={handleChange} onBlur={handleBlur} placeholder="https://..." />
                        {(touched.shopWebsite || submitCount > 0) && errors.shopWebsite && <div className="text-sm text-red-500">{errors.shopWebsite}</div>}
                      </div>
                    </div>
                  </div>
                        ...
                        <>
                          {values.requestItems?.map((item, idx) => {
                            ...
                            const handleImageUpload = async (event) => {
                              ...
                              try {
                                const baseLocal = item.localImages || []
                                const baseImgs = item.images || []
                                const newPreviews = files.map((f) => URL.createObjectURL(f))
                                // Optimistically show previews
                                setFieldValue(`requestItems[${idx}].localImages`, [...baseLocal, ...newPreviews])

                                // Upload sequentially and keep per-file results to maintain alignment
                                const results = []
                                for (const file of files) {
                                  const url = await uploadToCloudinary(file)
                                  results.push(url) // may be null on failure
                                }

                                const successfulUrls = results.filter((u) => !!u)
                                const successfulPreviews = newPreviews.filter((_, j) => !!results[j])

                                // Reconcile localImages to only keep previews for successful uploads
                                // Revoke any blob preview for failed uploads to avoid memory leaks
                                newPreviews.forEach((p, j) => {
                                  if (!results[j] && typeof p === 'string' && p.startsWith('blob:')) {
                                    URL.revokeObjectURL(p)
                                  }
                                })

                                setFieldValue(`requestItems[${idx}].localImages`, [...baseLocal, ...successfulPreviews])
                                setFieldValue(`requestItems[${idx}].images`, [...baseImgs, ...successfulUrls])
                              } catch {
                            ...
                            const handleRemoveImage = (imgIdx) => {
                              ...
                              const toRevoke = local[imgIdx]
                              if (typeof toRevoke === 'string' && toRevoke.startsWith('blob:')) {
                                try { URL.revokeObjectURL(toRevoke) } catch { void 0 }
                              }
                                ...
                                <CardContent className="p-4 space-y-4">
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                      ...
                                      {(touched.requestItems?.[idx]?.productName || submitCount > 0) && errors.requestItems?.[idx]?.productName && (
                                    ...
                                    <div className="space-y-2">
                                      ...
                                      {(touched.requestItems?.[idx]?.productURL || submitCount > 0) && errors.requestItems?.[idx]?.productURL && (
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    {(touched.requestItems?.[idx]?.quantity || submitCount > 0) && errors.requestItems?.[idx]?.quantity && (
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    {(touched.requestItems?.[idx]?.description || submitCount > 0) && errors.requestItems?.[idx]?.description && (
...
export default PurchaseRequestEditOffline
